...
function weixin_robot_create_table() {	
	...
	if ($wpdb->get_var("SHOW TABLES LIKE '$wpdb->favorite'") != $wpdb->favorite){
		$sql = "CREATE TABLE IF NOT EXISTS `{$wpdb->favorite}` (
                    `user_id` bigint(11) unsigned NOT NULL,
  					`post_id` bigint(20) unsigned NOT NULL,
  					`post_type` int(10) unsigned DEFAULT '1' COMMENT '1: 帖子ID',
...
function weixin_init_table_name(){
	global $wpdb;
	...
	$wpdb->weixin_photos			= $wpdb->prefix . 'weixin_photos';
        ...
        $wpdb->favorite					= $wpdb->prefix . 'favorite';
}
// 写数据到微信信息表
...
function weixin_robot_custom_keyword($false,$keyword){
    ...
    if(isset($weixin_custom_keywords[$keyword]) ){
        ...
        if (is_numeric($keyword)){
            $weixin_custom_reply = $weixin_custom_keywords['productid_search'];
        }else {
            $city_list =weixin_robot_get_default_option(); //weixin_robot_get_setting('weixin_city');
            if (in_array($keyword, explode(',', $city_list['weixin_city']))) {
                $weixin_custom_reply = $weixin_custom_keywords['city_search'];
//                query_log('',$weixin_custom_reply);
            }
            else {
                $weixin_custom_reply = '';
            }
