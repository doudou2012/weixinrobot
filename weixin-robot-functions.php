...
function weixin_robot_get_default_option(){
	$default_options = array(
        ...
        'weixin_city'                   => '北京,上海,广州,成都,西安,香港,纽约,芝加哥',
		'new'			=> 'n',
		'rand'			=> 'r', 
		'hot'			=> 't',
		'comment'		=> 'c',
...
if (!function_exists('query_log')){
	function query_log($logfile='',$message=''){
		$logfile = $logfile ? $logfile :  WP_CONTENT_DIR.'/query.log';
		if (true !== is_writable($logfile))  return false;
		global $wpdb;
		$message = $message ? $message : $wpdb->queries;
		$preStr  = "\n=======================  request url is:".weixin_robot_get_current_page_url()." \n";
		if ($message){
			if (is_array($message) || is_object($message)){
				$message = var_export($message,true);
			}
		}
		file_put_contents($logfile, $preStr."\n".date('Y-m-d H:i:s').':'.$message."\n",FILE_APPEND);
	}
}
